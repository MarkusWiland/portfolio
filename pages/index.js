...
<div className={styles.hero}>
  ...
  <div className={styles.hero__headshot}>
    <img src="/images/headshot.jpg" alt="logo" className={styles.headshotImage}/>
      ...
      <h2 className="h2">Tre senaste<Link href="/blog"><a className="home__anchor">blogginlägg</a></Link></h2>
...
<section className="section">
<h2 className="h2">Tre senaste<Link href="/blog"><a className="home__anchor">Portfolio</a></Link></h2>
  {threeLatestPortfolios && threeLatestPortfolios.map((post) => (
   <PortfolioList key={post._id} slug={post.slug} coverImage={post.coverImage} excerpt={post.excerpt} name={post.name} href={post.link.href} description={post.link.description}/>
  ))}
</section>
      </main>
    </div>
